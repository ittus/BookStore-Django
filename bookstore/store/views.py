    ...
    if request.user.is_authenticated():
        ...
        if processor == 'paypal':
            ...
            if payment.execute({'payer_id': payment.payer.payer_info.payer_id}):
                message = "Success! Your order has been completed, and is being processed. Paymetn ID: %s" % (payment.id)
                cart.active = False
            ...
            cart.save()
            message = "Success! Your order has been completed, and is being processed. Paymetn ID: %s" % (cart.payment_id)
            context = {
                'message': message
            }
            return render(request, 'store/order_complete.html', context)
